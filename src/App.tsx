...
export default function App() {
  ...
  const [userRole, setUserRole] = useState<"CITIZEN" | "OFFICER" | "WORKER" | "ADMIN">("CITIZEN");
  ...
  const [loginMode, setLoginMode] = useState<"citizen" | "staff">("citizen");
  ...
  // ── Auth helpers ──────────────────────────────────────────────────────────
  const persistSession = (token: string | null, user?: any) => {
    setAuthToken(token);
    try {
    ...
    if (user) {
      ...
      setUserCity(user.city || "Bengaluru");
      setUserRole((user.role as any) || "CITIZEN");
  ...
  const handleAuthFailure = (status: number) => {
    if (status === 401 || status === 403) {
      logout();
      setAuthError(status === 401 ? "Session expired. Please log in again." : "Access denied for your role.");
      return true;
    }
    return false;
  };

  // Restore session from stored JWT on load
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!authToken) {
      ...
      try {
        ...
        if (!cancelled) persistSession(authToken, user);
      } catch {
        if (!cancelled) persistSession(null);
      } finally {
  ...
  const handleSendOtp = async () => {
    setAuthError("");
    const phone = normalizePhone(loginInputPhone);
    if (!phone || phone.replace(/\D/g, "").length < 10) {
      setAuthError("Please enter a valid 10-digit Indian phone number.");
      return;
    }
    try {
      // Register if name provided and not yet registered; otherwise request OTP
      if (loginInputName.trim()) {
        const reg = await fetch("/api/auth/register", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            phone,
            name: loginInputName.trim(),
      ...
      if (!res.ok) {
        ...
        setAuthError(err.error || "Could not send OTP. Register first with your name.");
        return;
      }
      setOtpSent(true);
    } catch (e) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const res = await fetch("/api/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone, otp: otpCode || "123456" })
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        setAuthError(err.error || "Invalid OTP.");
        return;
      }
      const data = await res.json();
  ...
  const handleStaffLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        setAuthError(err.error || "Staff login failed.");
        return;
      }
      const data = await res.json();
      persistSession(data.token, data.user);
      ...
      console.error(e);
      setAuthError("Network error during staff login.");
    }
  };
  ...
  const handleSubmitIssue = async (e: React.FormEvent) => {
    ...
    try {
      if (!authToken || userRole !== "CITIZEN") {
        alert("Please log in as a citizen to report an issue.");
        return;
      }
      const payload = {
        title: formTitle,
  ...
  const handleWorkflowAction = async (payload: {
    ...
    try {
      if (!authToken) {
        alert("Please log in to update issue status.");
        return;
      }
      // Backend only accepts status / note / assignedDepartment; identity comes from JWT
      const body = {
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({}));
        alert(err.error || "Workflow update failed.");
      }
    } catch (err) {
  ...
  const handleAuthorityAction = async (status: IssueStatus, note: string, dept?: string) => {
    if (!selectedIssue) return;
    if (!authToken || (userRole !== "OFFICER" && userRole !== "ADMIN")) {
      alert("Officer login required to update status.");
      return;
    }
    try {
      const res = await fetch(`/api/issues/${selectedIssue.id}/status`, {
          ...
          {isLoggedIn ? (
            <div className="space-y-4">
              ...
              <div className="space-y-2 pt-1 border-t border-slate-800/60">
                <label className="text-[9px] uppercase font-bold text-slate-500 tracking-wider block">Signed in as</label>
                <div className={`text-center py-1.5 text-[10px] font-black rounded-lg border ${
                  userRole === "CITIZEN" ? "bg-blue-600/20 text-blue-300 border-blue-500/30" :
                  userRole === "OFFICER" || userRole === "ADMIN" ? "bg-purple-600/20 text-purple-300 border-purple-500/30" :
              ...
              <p className="text-xs font-bold text-slate-200">Sign in</p>
              <div className="grid grid-cols-2 gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800">
                <button
                  type="button"
                  onClick={() => { setLoginMode("citizen"); setAuthError(""); setOtpSent(false); }}
                  className={`py-1.5 text-[9px] font-black rounded-lg transition-all ${
                    loginMode === "citizen" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-white"
                  }`}
                >
                  Citizen OTP
                </button>
                <button
                  type="button"
                  onClick={() => { setLoginMode("staff"); setAuthError(""); }}
                  className={`py-1.5 text-[9px] font-black rounded-lg transition-all ${
              ...
              {authError && (
                <div className="bg-red-950/50 border border-red-800/50 text-red-300 text-[10px] p-2 rounded-lg">
                  {authError}
                </div>
              )}
              ...
              {loginMode === "citizen" ? (
                <>
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-slate-400 font-bold uppercase block">Full Name (new users)</label>
                    <input 
                      type="text" 
                  ...
                  {!otpSent ? (
                    ...
                    <div className="space-y-2">
                      <div className="bg-slate-800/80 p-2 rounded text-[10px] text-slate-300 border border-slate-700">
                        OTP sent. Demo code: <strong className="text-white">123456</strong>
                      </div>
                      <input 
                        type="password" 
                        value={otpCode}
                ...
                <>
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-slate-400 font-bold uppercase block">Work email</label>
                    <input 
                      type="email" 
                      value={staffEmail}
                      onChange={(e) => setStaffEmail(e.target.value)}
                      placeholder="vignesh.officer@bbmp.gov.in" 
                      className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-purple-500"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-slate-400 font-bold uppercase block">Password</label>
                    <input 
                      type="password" 
                      value={staffPassword}
                  ...
                  <button 
                    ...
                    className="w-full py-2 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs rounded-lg transition-all"
                  >
                    Staff sign in
                  </button>
                  <p className="text-[9px] text-slate-500 leading-relaxed">
                    Demo: vignesh.officer@bbmp.gov.in / changeme123
                  </p>
                </>
              )}
            </div>
